...
- Mobile and tablet layouts keep the wheel and import/export dialog within the viewport without scrolling or browser zoom; the import controls compact to fit alongside the export tab's layout
...
- Local default: `1.9.4` (set in `BookWheel/BookWheel.csproj`)
...
```bash
dotnet build BookWheel.slnx /p:InformationalVersion=1.9.4
docker build --build-arg APP_VERSION=1.9.4 -t jasonkryst/bookwheel:1.9.4 .
```

## Automated Docker Publish on Version Release

GitHub Actions publishes Docker images to Docker Hub and GHCR when a GitHub Release is published (for example, tagged `v1.9.4`).
...
- `jasonkryst/bookwheel:<version-without-v>` (for example `1.9.4`)
- `jasonkryst/bookwheel:latest` (only for non-prerelease versions)
- `ghcr.io/jasonkryst/bookwheel:<version-without-v>` (for example `1.9.4`)
