...
- Mobile and tablet layouts keep the wheel and import/export dialog within the viewport without scrolling or browser zoom
...
- Local default: `1.9.3` (set in `BookWheel/BookWheel.csproj`)
...
```bash
dotnet build BookWheel.slnx /p:InformationalVersion=1.9.3
docker build --build-arg APP_VERSION=1.9.3 -t jasonkryst/bookwheel:1.9.3 .
```

## Automated Docker Publish on Version Release

GitHub Actions publishes Docker images to Docker Hub and GHCR when a GitHub Release is published (for example, tagged `v1.9.3`).
...
- `jasonkryst/bookwheel:<version-without-v>` (for example `1.9.3`)
- `jasonkryst/bookwheel:latest` (only for non-prerelease versions)
- `ghcr.io/jasonkryst/bookwheel:<version-without-v>` (for example `1.9.3`)
