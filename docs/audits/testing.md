...
**Status (2026-09-06): Fixed.** All 5 affected classes now implement `IClassFixture<BookWheelWebAppFactory>` + `IAsyncLifetime`, sharing one Testcontainer per class with a `ResetAsync()` truncation before each test (`BookWheelHealthCheckTests` needed no change — confirmed it held no per-test container instantiations). Two structural side effects of sharing one app host per class had to be fixed alongside the container-sharing itself, both invisible until measured against the full shared-class test volume: (1) the in-process login IP rate limiter and per-username lockout counter in `AuthService` now persist across all tests in a class instead of resetting per test, so ordinary tests were tripping 429/423 responses meant only for the two dedicated rate-limit tests — fixed by giving every ordinary login call a distinct synthetic `X-Forwarded-For` IP and raising the test-only username-lockout threshold; (2) a log-assertion test (`Failed_Login_Is_Recorded_As_Structured_Warning_Log`) was matching "last/first entry for this category+message" against a now-shared `LoggerProvider`/log file that other tests also write to — fixed by using a unique username and filtering on it explicitly rather than positional "last/first". Full suite now runs in ~1.25 minutes (verified stable across two consecutive full runs, 284/284 passed both times), down from the 34.3-minute baseline.
