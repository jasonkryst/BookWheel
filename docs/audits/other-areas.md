...
**Status (2026-09-07, GH #92): resolved.** `BookWheel/appsettings.json` now ships `Analytics:GoogleAnalyticsId: ""` by default (no hardcoded property ID committed to source), `Program.cs`/`wwwroot/index.html` skip emitting the `gtag.js` script entirely when the ID is empty, and `README.md`'s "Analytics" section documents the setting, its `Analytics__GoogleAnalyticsId`/`GOOGLE_ANALYTICS_ID` environment-variable overrides, and the blank-by-default behavior. See `SECURITY_AUDIT_REPORT.md` Finding #2 for the full history — the note below is preserved as a record of the original finding.

Original finding (2026-09-04): `BookWheel/appsettings.json` shipped a hardcoded default `Analytics:GoogleAnalyticsId: "G-JQXW826H1F"`, which `Program.cs` injected into `wwwroot/index.html` (replacing `__GOOGLE_ANALYTICS_ID__`) and which loaded `googletagmanager.com/gtag/js` unconditionally in every deployment that didn't override the setting. Neither `README.md` nor any other doc in the repo mentioned `Analytics`, `GoogleAnalyticsId`, or Google Analytics at all — it wasn't listed in the "Features" list, the configuration/settings guidance, or anywhere in "Getting Started"/"Container Support." For the open-source angle that mattered here: anyone who cloned/forked the repo and deployed via `docker run`, `docker-compose up`, or `dotnet run` without reading `appsettings.json` source would silently start sending their users' page-view telemetry to the original maintainer's GA property, with no README section, environment-variable callout, or setup-checklist item telling them this setting existed, what it did, or how to blank it out.
